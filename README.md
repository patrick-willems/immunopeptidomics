...
## External tools
This processing is currently developed using tools listed below that have to be downloaded/installed. Not all tools are required to run part of the pipeline workflow. MSFragger is required for preprocessing RAW/.d data to mzML - which is used for searching identical MS2 spectra with the other engines. In addition, ms2rescore and mokpaot perform uniform rescoring of all search engine results, which facilitates downstream integration. 
